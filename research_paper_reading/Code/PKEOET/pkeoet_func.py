...
def main(args):
    ...
    logger = Logger(
        "/home/seed/Documents/ResearchPaper/PKEOET/output/log2.txt", title="PKEOET")
    logger.set_names(['Cipher Num', 'Enc Time', 'PTest Time',
                     'Total Time P','DTest Time' ,'Total Time'])
    ...
    with open('/home/seed/Documents/ResearchPaper/PKEOET/ciphertexts/c_gt.txt', 'r') as f:
        for l in f.readlines():
            ...
            dTestTimestart = time.time()

            dTest(v)

            dTest_time += time.time() - dTestTimestart

            if count == nums[cursor]:
                print('Enc %d Time: %s' % (count, enc_time))
                print('pTest %d Time: %s' % (count, pTest_time))
                print('Total Time: ', total_time)
                logger.append([count, enc_time, pTest_time, total_time,dTest_time, total_time+dTest_time])
                cursor += 1
